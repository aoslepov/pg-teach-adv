...
--- выключаем кластер постгрес и отмонтируем каталог с тестовыми данными на ВМ pg-teach-01
sudo -u postgres pg_ctlcluster 15 main stop
umount /mnt/data/

--- проверяем установленную версию постгрес на pg-teach-01
dpkg -l | grep postgres-15
ii  postgresql-15                         15.2-1.pgdg22.04+1

-- отмонтируем дополнительный диск от ВМ pg-teach-01

-- заказываем новую ВМ pg-teach-02
yc compute instance create \
  --name pg-teach-02 \
...
-- монтируем дополнительный диск к ВМ pg-teach-02

--- ставим на ВМ pg-teach-02 postgres-15
...
--- останавливаем кластер на ВМ pg-teach-02
sudo -u postgres pg_ctlcluster 15 main stop
sudo -u postgres pg_lsclusters
Ver Cluster Port Status Owner    Data directory               Log file
...
--- проверям что диск подмонтирован к ВМ pg-teach-02
fdisk -l
...
--- проверяем тестовый набор нанных на ВМ pg-teach-02
sudo -u postgres psql
