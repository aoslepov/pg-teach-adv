...
Генерируем сертификаты на перпой ноде
```
cdb-01 >>
cd /opt/cockroach/
...
```
for i in {'62.84.117.81','84.252.141.37','51.250.47.60'}; do
ssh ubuntu@$i './opt/cockroach/cockroach start --certs-dir=/opt/cockroach/cockroach/certs --advertise-addr=cdb-03 --join=cdb-01,cdb-02,cdb-03 --cache=.25 --max-sql-memory=.25 --background'
done
```
